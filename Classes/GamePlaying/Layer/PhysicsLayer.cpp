...
void PhysicsLayer::onEnter()
{
    ...
    EnemyPlaneController::getInstance()->startLevelWithConfigFile(GameData::getInstance()->getPlaneManager()->getCurLvl());
...
void PhysicsLayer::delayAddPlane()
{
	...
	GameData::getInstance()->getPlaneFactory()->CreateMainPlane((PlaneType) curRole);

	auto mainpl = GameData::getInstance()->getMainPlane();
	int protectNum = NewDataMgr::getInstance()->mGameBuySystem->getBuyGoodsDataWithKey(BuyListKey::BuyKey_HuDun)->number;
	if (protectNum > 0)
	{
		//护盾
		if (mainpl)
		{
			mainpl->AddProtect();
			NewDataMgr::getInstance()->mGameBuySystem->addGoodsNumWithKey(BuyListKey::BuyKey_HuDun,-1);
		}
	}
}
...
{
	NewAudio::getInstance()->playEff(Music_BossIn);
